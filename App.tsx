...
function App() {
  ...
  return (
    <div className="min-h-screen pb-10 text-slate-800 dark:text-slate-200 selection:bg-blue-500/30 selection:text-blue-600 dark:selection:text-blue-200 transition-colors duration-300 bg-[url('https://grainy-gradients.vercel.app/noise.svg')]">
        {!userId && <LandingHeader onDev={() => setShowDevModal(true)} onLogin={() => auth.setShowLoginModal(true)} onGuide={() => setShowAppGuide(true)} theme={settings.theme} onToggleTheme={toggleTheme} />}
        <main className="container max-w-screen-xl mx-auto py-6 px-4">
            {userId ? (
                <>
                    <DashboardHeader onDev={() => setShowDevModal(true)} status={connectionStatus} userId={userId} userData={userData} onLogout={handleLogout} onToggleTheme={toggleTheme} theme={settings.theme} onGuide={() => setShowAppGuide(true)} />
                    {isLoading ? (
                        ...
                        <>
                            <div className="no-print"><HeroSection compositeData={compositeData} streak={streak} settings={settings} onUpdateWeights={dataMgr.handleWeightUpdate} onUpdateCountdown={(t, l) => handleSettingsUpdate({ ...settings, countdownTarget: t, countdownLabel: l })} /></div>

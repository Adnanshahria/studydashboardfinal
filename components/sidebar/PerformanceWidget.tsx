        ...
        <div className="glass-panel p-5 rounded-3xl">
            ...
            <div className="flex flex-col gap-6">
                {settings.progressBars.filter(conf => conf.visible !== false).map(conf => {
                    ...
                    return (
                        <div key={conf.id}>
                            <div className="flex justify-between text-xs font-bold mb-2 text-slate-700 dark:text-slate-300"><span>{conf.title}</span><span className="text-blue-600 dark:text-blue-400">{p.overall.toFixed(1)}%</span></div>
                            <ProgressBar progress={p.overall} color={`bg-gradient-to-r ${conf.color}`} />
                            <div className="flex justify-between text-[10px] text-slate-500 mt-1.5 px-0.5"><span className="opacity-70">P1: {p.p1.toFixed(0)}%</span><span className="opacity-70">P2: {p.p2.toFixed(0)}%</span></div>
                        </div>
                    );
                })}
            </div>
